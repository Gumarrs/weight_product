...
<?php
	...
	if(isset($_GET['nilai_user'])){
		...
		<div>
<h1 style="text-align:center;">Dengan Rincian Perhitungan Berikut</h1>
<?php
...
for($i=0;$i<$jml_pelamar;$i++){
    // Ambil nama lengkap berdasarkan ID pengguna
    $qrU = $user->GetData("where id_user='$ar_pelamar[$i]'");
    $rowU = $qrU->fetch();
    $nama_lengkap = $rowU['nama_lengkap'];

    $loop2 = $pelamar->GetData("where id_lowongan='$id_lowongan' and id_user='$ar_pelamar[$i]' and nilai > 0 order by kriteria asc");
    echo "<b>Pelamar dengan nama " . $nama_lengkap . "</b><br>";
    $ib = 0;
    $vkt_s = 1;
    while($k = $loop2->fetch()){
    ...
    echo "Vektor s pelamar " . $nama_lengkap . " = " . $vkt_s . "<br>";
    $qryVS = $hitung->SetVektor_S($vkt_s, $ar_pelamar[$i], $id_lowongan);
    $total_vektor_s += $vkt_s;
}

// Tampilkan nilai Vektor V
for($i=0;$i<$jml_pelamar;$i++){
    // Ambil nama lengkap berdasarkan ID pengguna
    $qrU = $user->GetData("where id_user='$ar_pelamar[$i]'");
    $rowU = $qrU->fetch();
    ...
    // Ambil nilai vektor_s dari database
    $qrVS = $hitung->GetData("where id_user='$ar_pelamar[$i]' and id_lowongan='$id_lowongan'");
    $rowVS = $qrVS->fetch();
    $vektor_s = $rowVS['vektor_s'];

    $vektor_v = $vektor_s / $total_vektor_s;
    
	echo "<hr><br>";
    echo "Vektor v pelamar " . $nama_lengkap . " = " . $vektor_v . "<br>";
    $qryVV = $hitung->SetVektor_V($vektor_v, $ar_pelamar[$i], $id_lowongan);
}
